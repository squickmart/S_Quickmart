    ...
    window.applyStoreStatus = function () {
      ...
      // True when orders can be placed right now: normal hours, or the
      // admin has force-opened the store via "Store Open Override".
      function isStoreOrderable() {
        return isOpen() || window._storeOpen === true;
      }
      ...
      function setupTiming() {
        ...
        if (isOpen()) {
          ...
          // NOTE: closedOverlay display is handled by applyStoreStatus(),
          // which also respects the "Store Open Override" force-open flag.
          // Do not force-show it here based on time alone.
        }
      }
      ...
      function updateCheckoutClosedNotice() {
        const notice = document.getElementById("checkoutClosedNotice");
        if (!notice) return;
        notice.style.display = isStoreOrderable() ? "none" : "block";
      }
      ...
      async function placeOrder() {
        if (!isStoreOrderable()) {
          showToast("❌ Store band hai! Order sirf 7AM–10PM ke beech hi le sakte hain.");
          return;
        }
        ...
        // Save/update user details for next time (also keeps the admin
        // panel's customer profile in sync if the name/address was
        // edited here at checkout, not just from the Profile screen).
        currentUser = {
          phone,
          name,
          address: fullAddr,
          area: selectedCheckoutArea || currentUser.area || "",
        };
        localStorage.setItem("sqm_user", JSON.stringify(currentUser));
        updateProfileUI();
        try {
